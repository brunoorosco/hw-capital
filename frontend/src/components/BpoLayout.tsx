...
const bpoMenuItems = [
  ...
  { icon: BarChart3, label: "Conciliação Bancária", path: "/bpo/reconciliation" },
...
export default function BpoLayout({ children }: BpoLayoutProps) {
  ...
  const isDiretoriaPage = location === "/bpo/plans" || location === "/bpo/users";
    ...
    <div className="min-h-screen bg-ivory">
      ...
      <TooltipProvider delayDuration={0}>
        ...
        <div className="p-6 border-b-2 border-gold/30 relative">
          ...
          {!sidebarCollapsed && <p className="text-cream text-sm mt-1">BPO Financeiro</p>}
          ...
          {(() => {
            ...
            const menuItem = (
              <Link href={perfilMenuItem.path}>
                ...
                >
                  <Icon className="w-5 h-5 shrink-0" />
                  {!sidebarCollapsed && <span className="font-medium">{perfilMenuItem.label}</span>}
                </motion.div>
              </Link>
            );
      ...
      <header className="lg:hidden fixed top-0 left-0 right-0 z-40 bg-emerald-dark border-b-2 border-gold/30 px-4 py-3">
        <div className="flex items-center justify-between">
          ...
          <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
            ...
            <SheetContent side="left" className="w-80 bg-emerald-dark border-gold/30 p-0">
      ...
      {/* Main Content */}
      <main className={`transition-all duration-300 pt-16 lg:pt-0 ${sidebarCollapsed ? 'lg:ml-20' : 'lg:ml-72'}`}>
